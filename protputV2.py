...
#OTM
    ...
    strike = st.selectbox("Select strike (USD): ",options=otm_values,index=int(len((otm_values))/1.2))
#Spec Strike
...
if purpose_select==purpose[0]:
    ...
    if build_pos=="Yes":
        nr_shares=st.number_input("Enter amount of equity held in quantity (Shares)/units (ETF):",min_value=0,step=5,value=500)
        ...
        if  nr_contracts_buy==0:
            ...
            st.write("The take profit price for the Put contracts is: ",round(last_price*(1+perc_profit),2))
            st.write("The profit amount would be: ",round((last_price*(perc_profit))*100*nr_contracts_buy,2),"USD including premium paid.")
            st.write("The stop loss price for the Put contracts is: ",round(last_price*(1-perc_risk),2))
            st.write("The loss amount would be: ",round((last_price*(perc_risk))*100*nr_contracts_buy,2),"USD including premium paid.")
...
if purpose_select==purpose[1]:
    ...
    if  nr_contracts_buy==0:
        ...
        st.write("The take profit exit price is: ",round(entry_price*(1+perc_profit),2))
        st.write("The profit amount would be: ",round((entry_price*(perc_profit))*100*nr_contracts_buy,2),"USD including premium paid.")
        st.write("The stop loss price is: ",round(entry_price*(1-perc_risk),2))
        st.write("The loss amount would be: ",round((entry_price*(perc_risk))*100*nr_contracts_buy,2),"USD including premium paid.")
